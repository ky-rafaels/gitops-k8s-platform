...
if hostname -I 2>/dev/null; then
  myip=$(ip addr show enp0s1  | awk '$1 == "inet" { print $2 }' | cut -d/ -f1)
else
  myip=$()
fi
...
networking:
  ...
  serviceSubnet: "172.$(echo $twodigits | sed 's/^0*//').0.0/16"
  podSubnet: "172.1${twodigits}.0.0/16"
